function[Mr,Mc,e_r_left,e_r_rate] = ...
    Bragg_params_heat_lengthways(free_space_lambda,n_ext,material,...
    len_abs,r,dom_x,idx,lWG,rWG,T_diff)
...
dx = xd(2)-xd(1);
% Heat source placed on the left, assume room temperature is 20C
% T = 80; % degrees C 
dT = T_diff;
dndT = 1.86e-6; % dn/dT (K^{-1})
e_r_left = (sqrt(e_r) + dT * dndT)^2;
e_r_right  = e_r;

% %% Varying the temperature across the face (yz) of the grating
% e_r_rate = (e_r_right-e_r_left)/(M*dx); 
% y_shift = coords(1,1,1);
% E_R = e_r_left + (coords(:,:,1)-y_shift)*e_r_rate;
...
e_r_rate = (e_r_right - e_r_left)/(x_Bragg_end - x_Bragg_start);
